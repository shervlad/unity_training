...
public class GridController : MonoBehaviour
{
    ...
    void setUp(){
        ...
        float x,y,z;
        for(int j=0;j<gridSize;j++){
            y = transform.localPosition.y - gridSize*cellSize/2 + j*cellSize;
            for(int k=0;k<gridSize;k++){
                z = transform.localPosition.z + k*cellSize;
                for(int i=0;i<gridSize;i++){
                    x = transform.localPosition.x -gridSize*cellSize/2 + i*cellSize;
                    cells[i,k,j] = Instantiate(prefabCell, new Vector3(0, 0, 0), Quaternion.identity);
                    cells[i,k,j].transform.parent = transform;
                    cells[i,k,j].transform.localPosition = new Vector3(x,y,z);
                }
            }
        }
    }
